...
studentsGroups["ІВ-81"] = [];
studentsGroups["ІВ-82"] = [];
studentsGroups["ІВ-83"] = [];
studentsGroups["ІО-81"] = [];
studentsGroups["ІО-82"] = [];
studentsGroups["ІО-83"] = [];
studentsGroups["ІП-83"] = [];
studentsGroups["ІП-84"] = [];
let studentInfo:[String] = studentsStr.components(separatedBy: "; ")
var cmp:[String] = []
for student in studentInfo {
    cmp = student.components(separatedBy: " - ");
    studentsGroups[cmp[1]]?.append(cmp[0]);
}
...
for (group, students) in studentsGroups {
    ...
    for student in students{
        ...
        for points in points {
            grad.append(randomValue(maxValue: points))
        }
        studentPoints[group]![student] = grad;
    }
}
...
for (group, students) in studentPoints {
    sumPoints[group] = [:];
    for student in students{
        gradSum = student.value.reduce(0, +)
        sumPoints[group]![student.key] = gradSum;
        gradSum = 0;
    }
}
...
for (group, students) in sumPoints {
    for student in students{
        gradSum += student.value;
    }
    groupAvg[group] = Float(gradSum)/Float(students.count);
    gradSum = 0;
}
...
for (group, students) in sumPoints {
    ...
    for student in students{
        if student.value > 60 {
            passedPerGroup[group]!.append(student.key);
        }
    }
}
